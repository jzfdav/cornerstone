...
import { supabase, type User, type Project } from '../lib/supabase'
import { validateDemoProjectCode, validateDemoLoginCode } from '../lib/demoAuth'
...
export function useAuth() {
    ...
    const validateProjectCode = useCallback(async (code: string): Promise<{ valid: boolean; project?: Project }> => {
    ...
    const login = useCallback(async (
        ...
        accessCode: string,
        rememberMe: boolean
    ): Promise<{ success: boolean; projectError?: boolean; accessError?: boolean }> => {
        // Validate project code
        ...
        localStorage.setItem(STORAGE_KEYS.PROJECT_CODE, projectCode.trim().toUpperCase())

        // Validate access code
        const userResult = await validateAccessCode(accessCode, projectResult.project.id)
